...
const filters = [
  ...
  { key: 'region', label: 'Region', type: 'text', placeholder: 'e.g. NWE, AG' },
...
export default function Flows() {
  ...
  const { data, isLoading, error, refetch } = useQuery({
    ...
    queryFn: () => api.getFlows(queryParams),
    enabled: !!queryParams,
  });
      ...
      {!queryParams && !isLoading && (
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-6 text-center">
          ...
          <p className="text-blue-500 text-sm mt-1">
            Filter by vertical, region, ports, and date range
          </p>
        </div>
      )}

      {isLoading && <LoadingSpinner message="Fetching flows data..." />}
